...
class Solution:
    def findMinArrowShots(self, points: List[List[int]]) -> int:
        points.sort(key=lambda x: x[1])  # 对points进行排序，以xend升序排列
        last_end = points[0][1]  # 初始化前一无重叠区间的末位元素
        count = 1  # 至少有一个无重叠区间
        for i in range(len(points)):  # 遍历所有区间
            start = points[i][0]
            if start > last_end:  #若发现无重叠区间，则计数器加1
                count += 1
                last_end = points[i][1]  # 更新last_end，将当前无重叠区间的xend作为last_end
        return count
